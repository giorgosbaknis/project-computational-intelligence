...
class NeuralNet(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, rin=0.8, rh=0.2):
        super(NeuralNet, self).__init__()
        self.hidden1 = nn.Linear(input_size, hidden_size)
        self.relu1 = nn.ReLU()
        self.dropout1 = nn.Dropout(rin) # Dropout for entry nodes
        self.hidden2 = nn.Linear(hidden_size, hidden_size//2) # Second hidden layer
        self.relu2 = nn.ReLU()
        self.dropout2 = nn.Dropout(rh) # Dropout for hidden nodes
        self.hidden3 = nn.Linear(hidden_size//2, hidden_size//4) # Third hidden layer
        self.relu3 = nn.ReLU()
        self.dropout3 = nn.Dropout(rh) # Dropout for hidden nodes
        self.output = nn.Linear(hidden_size//4, output_size)
        
    def forward(self, x):
        x = self.dropout1(self.relu1(self.hidden1(x)))
        x = self.dropout2(self.relu2(self.hidden2(x))) # Pass through the second hidden layer with dropout
        x = self.dropout3(self.relu3(self.hidden3(x))) # Pass through the third hidden layer with dropout
        x = self.output(x)
        return x
